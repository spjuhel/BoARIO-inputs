...
NAME_RE = r"(?P<xp_name>[a-zA-Z\-\d]+)~(?P<mrio_name>[a-zA-Z\-_\d]+)~(?P<params>psi_(?P<psi>(?:0|1)_\d+)_order_(?P<order>[a-z]+)_inv_(?P<inv>\d+)_reb_(?P<reb>\d+)_evtype_(?P<evtype>[a-zA-Z]+))~(?P<region>[A-Z]{2,3}|[A-Z]{2}\d+)~(?P<class>(?:\d+%)|max|min).name"
